...
import morganMiddleware from './core/morganMiddleware';
import express, { NextFunction, Request, Response } from 'express';
import Logger from './core/logger';
import { NODE_ENV, PORT, ORIGIN, CREDENTIALS } from './config';
import { dbConnection } from './databases';
import { V1AllRouter } from './routes/_all.route';
import { SwaggerRouter } from './helper/swagger-routes';
import { ReportsRouter } from './helper/reports-routes';
...
class App {
  ...
  private initializeRoutes() {
    ...
    this.app.get('/', (req: Request, res: Response) => {
      return res.send(
        'Welcome to API service. Last build happened on - ' + date
      );
    });

    this.app.use('/api/v1', new V1AllRouter().router);
